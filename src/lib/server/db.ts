...
const DEFAULT_SETTINGS: Settings = { slogan: "calirko's personal log", backgroundKey: null, accentColor: null };
...
async function getDb(): Promise<Low<DB>> {
	if (!_db) {
		...
		_db = new Low<DB>(adapter, { posts: [], settings: { ...DEFAULT_SETTINGS } });
		await _db.read();
